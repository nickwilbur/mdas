...
export function getMockDataPrior(): {
  ...
  // IBM (idx 5): prior renewal stage Stage 2; current Stage 3+
  const ibmRenewal = prior.opportunities.find((o) => o.accountId === accounts[5]?.accountId);
  if (ibmRenewal) {
    ibmRenewal.stageName = 'Stage 2 - Discovery';
    ibmRenewal.stageNum = 2;
    ibmRenewal.forecastMostLikely = 150_000;
  }
