...
export interface ForecastInput {
  ...
  audience?: string;
  /**
   * Optional managerial input for the selected quarter. When provided,
   * the generator computes Gap to Plan; otherwise a `[fill in]` placeholder
   * is emitted so leadership knows the field is intentionally blank.
   */
  ...
  /**
   * Qualitative narrative (3–5 sentences) for the selected quarter,
   * generated upstream via Glean Adaptive chat. Rendered as a
   * `Health Snapshot:` block between the KPI lines and account sections.
   * When undefined or whitespace-only the block is omitted.
   */
...
/**
 * MDAS-estimated Flash — renewal downsell/churn dollars for the quarter
 * (open **and** closed). Matches Clari Forecast → Churn & Downsell →
 * **Total** tab footer on Forecast Most Likely (account ML roll-up), not
 * the separate manager weekly "Churn & Downsell Flash" line in Clari
 * forecast exports. Plan / Hedge still come from Clari CSV when pasted.
 */
function mdasAccountFlashChurnUSD(rows: QuarterBucket['rows']): number {
  return rows.reduce((s, r) => {
    if (!isRenewalDownsellChurnOpp(r.view, r.opp)) return s;
    return s + opportunityChurnFlashUSD(r.opp);
  }, 0);
}

export interface ChurnOpportunitySupplement {
  accounts: CanonicalAccount[];
  opportunities: CanonicalOpportunity[];
}

/**
 * Back-fill renewal downsell/churn opportunities dropped from the latest
 * refresh but still present in a recent prior snapshot. Closes the gap
 * between MDAS Flash and the Clari churn grid when SFDC transiently
 * omits in-quarter churn rows between refreshes.
 */
export function supplementViewsWithDroppedQuarterChurnOpps(
  views: AccountView[],
  prior: ChurnOpportunitySupplement,
  asOfDate: string,
  buildView: (
  ...
  const viewsByAccount = new Map(views.map((v) => [v.account.accountId, v]));
  const currentOppIds = new Set(
    views.flatMap((v) => v.opportunities.map((o) => o.opportunityId)),
  );
  ...
  for (const opp of prior.opportunities) {
    if (currentOppIds.has(opp.opportunityId)) continue;
    const oppFq = fiscalQuarterFromDate(opp.closeDate);
    if (!oppFq || oppFq.key !== fq.key) continue;
    if (!isRenewalLike(opp) || !hasDownForecastSignal(opp)) continue;

    const account = accountsById.get(opp.accountId);
    if (!account) continue;

    currentOppIds.add(opp.opportunityId);
    const existing = viewsByAccount.get(opp.accountId);
    if (existing) {
      viewsByAccount.set(opp.accountId, {
        ...existing,
        opportunities: [...existing.opportunities, opp],
      });
    } else {
      viewsByAccount.set(opp.accountId, buildView(account, [opp]));
    }
  }

  return [...viewsByAccount.values()];
}
...
/** Per-refresh KPI pair stored on refresh_runs.trajectory_kpis (plan/gap null). */
export interface RefreshTrajectoryKpis {
  /** Calendar date (YYYY-MM-DD) derived from the refresh started_at. */
  asOfDate: string;
  current: QuarterKpiSnapshot;
  next: QuarterKpiSnapshot;
}

/** Compute trajectory KPIs from scored views; Plan is stored null for later overlay. */
export function computeRefreshTrajectoryKpis(
  views: AccountView[],
  asOfDate: string,
): RefreshTrajectoryKpis {
  return {
    asOfDate,
    current: computeQuarterKpis(views, asOfDate, 'current', null),
    next: computeQuarterKpis(views, asOfDate, 'next', null),
  };
}

/** Apply caller-supplied Plan dollars to a stored KPI snapshot. */
export function applyTrajectoryPlan(
  kpis: QuarterKpiSnapshot,
  planUSD: number | null | undefined,
): QuarterKpiSnapshot {
  if (planUSD == null) return kpis;
  return {
    ...kpis,
    planUSD,
    gapUSD: kpis.flashUSD - planUSD,
  };
}
...
function accountMlMismatchGapUSD(group: MlMismatchAccountGroup): number {
  return group.opps.reduce((s, o) => s + Math.abs(o.gapUSD), 0);
}

function formatMlMismatchGapUSD(gapUSD: number): string {
  return `${fmtUSD(gapUSD)} gap`;
}
...
function renderMlOverrideMismatchSection(
  ...
  for (const group of groups) {
    ...
    const gapLabel = formatMlMismatchGapUSD(accountMlMismatchGapUSD(group));
    if (enr?.unavailableReason) {
      headlines.push(`${accountName} (${gapLabel}) — [context unavailable]`);
      continue;
    }
    if (!enr) {
      headlines.push(`${accountName} (${gapLabel})`);
      continue;
    }
    const rollup = formatMlMismatchRollupHeadline(accountName, enr);
    if (rollup) headlines.push(`${rollup} (${gapLabel})`);
    else headlines.push(`${accountName} (${gapLabel})`);
  }
  ...
  for (const group of groups) {
    ...
    const enr = enrichments?.[primary.opp.opportunityId];
    lines.push(
      `${group.view.account.accountName} (${formatMlMismatchGapUSD(accountMlMismatchGapUSD(group))})`,
    );
...
function renderQuarterSection(
  ...
  lines.push(`Quarter: ${bucket.label}`);
  lines.push(
  ...
  const mlMismatchGroups = groupMlMismatchByAccount(mlMismatchOpps);
  const mlMismatchTotal = mlMismatchGroups.reduce(
    (s, g) => s + accountMlMismatchGapUSD(g),
    0,
  );
  lines.push(
  ...
  // Red — risk trending accounts in the selected quarter.
 ...
 *   - One quarter: the fiscal quarter containing `asOfDate` (the
 *     quarter selected in the forecast UI).
