# Expand 3 CTA Scan — 2026-06-25

**Generated:** 2026-06-25T23:13:26.859Z
**Total CTAs:** 31
**Renewal scope:** FY27 + FY28 open renewals
**CTA gate:** dark, identified risk, or unhealthy only
**Script:** `scripts/generate-ctas.ts` (v3 — @mdas/cta-engine)

---
## CTA 1 — Antylia Scientific

```json
{
  "cta_id": "expand3-2026-06-25-antylia-scientific-dark_account",
  "account_name": "Antylia Scientific",
  "salesforce_account_id": "0017000001IFGK1AAP",
  "play_type": "dark_account",
  "risk_color": "Red",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B35PDQGUC",
  "renewal_opportunity_id": "006Po00000l1gIHIAY",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000l1gIHIAY/view",
  "drivers": [
    "Renewal date: 2026-07-05",
    "ARR: $94,164",
    "ATR at risk: $47,082",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 1 min (30d) — below threshold",
    "CSE Sentiment: Red"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-02",
  "check_back_date": "2026-06-25",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-06-25",
    "auto_check_query": "Antylia Scientific Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-02",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p color=\"\" style=\"\">State and Renewal:<br style=\"\">Although current usage remains ok, Year-over-Year (YoY) results are weak. The customer seems less engaged, having declined recent Table Talk invitations, mentioning no interest for now.There is renewal risk</p><p color=\"\" style=\"\">Account Plan:</p><p color=\"\" style=\"\">we’ve reached out to Ashish Patel, Director of Software Engineering, to set up an account review. no response.Follow ups are done</p>",
  "commentary_last_updated": "2026-06-22T06:36:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 101,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 1 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000l1gIHIAY:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 47082,
  "renewal_opportunity_name": "Antylia Scientific Billing 07-26"
}
```

---

## CTA 2 — Traxxall

```json
{
  "cta_id": "expand3-2026-06-25-traxxall-dark_account",
  "account_name": "Traxxall",
  "salesforce_account_id": "0010g00001aLjB5AAK",
  "play_type": "dark_account",
  "risk_color": "Red",
  "primary_owner": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cc_owners": [],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B32DM1LLW",
  "renewal_opportunity_id": "006Po000012adUJIAY",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po000012adUJIAY/view",
  "drivers": [
    "Renewal date: 2026-12-22",
    "ARR: $39,000",
    "ATR at risk: $19,500",
    "CSE sentiment commentary last updated 150d ago",
    "No dedicated CSE (digital coverage)",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 1 min (30d) — below threshold"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Traxxall Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [
    "No CSE assigned (digital coverage)"
  ],
  "cse_sentiment_commentary": "<p>Confirmed churn. Zuora will no longer be the system of record for the Customer as of the renewal or the Customer has already migrated off of Zuora ahead of their renewal date (could include a “read-only” tenant)</p>",
  "commentary_last_updated": "2026-01-26T07:58:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cse": null,
  "priority_score": 96,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "CSE sentiment commentary last updated 150d ago",
      "observedAt": "2026-01-26T07:58:00.000+0000"
    },
    {
      "source": "salesforce",
      "signal": "No dedicated CSE (digital coverage)"
    },
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 1 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po000012adUJIAY:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 19500,
  "renewal_opportunity_name": "Traxxall Billing 12-26"
}
```

---

## CTA 3 — Luminary Media

```json
{
  "cta_id": "expand3-2026-06-25-luminary-media-dark_account",
  "account_name": "Luminary Media",
  "salesforce_account_id": "0010g00001cVNjCAAW",
  "play_type": "dark_account",
  "risk_color": "Red",
  "primary_owner": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cc_owners": [],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B2P5VCJ2K",
  "renewal_opportunity_id": "006Po00000wCPpFIAW",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000wCPpFIAW/view",
  "drivers": [
    "Renewal date: 2026-10-17",
    "ARR: $86,400",
    "ATR at risk: $43,200",
    "CSE sentiment commentary last updated 142d ago",
    "No dedicated CSE (digital coverage)",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 1 min (30d) — below threshold"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Luminary Media Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [
    "No CSE assigned (digital coverage)"
  ],
  "cse_sentiment_commentary": "<p></p><p color=\"\" style=\"\">State and renewal :</p><p color=\"\" style=\"\">Customer renewed for next one year.Since the last renewal was not smooth,upcomimg renewal is in risk.</p><p color=\"\" style=\"\">Account Plan:</p><p color=\"\" style=\"\">Need to engage with the customer to address the challenges.</p>",
  "commentary_last_updated": "2026-02-03T07:15:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cse": null,
  "priority_score": 95,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "CSE sentiment commentary last updated 142d ago",
      "observedAt": "2026-02-03T07:15:00.000+0000"
    },
    {
      "source": "salesforce",
      "signal": "No dedicated CSE (digital coverage)"
    },
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 1 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000wCPpFIAW:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 43200,
  "renewal_opportunity_name": "Luminary Media Billing 10-26"
}
```

---

## CTA 4 — Missional Marketing

```json
{
  "cta_id": "expand3-2026-06-25-missional-marketing-dark_account",
  "account_name": "Missional Marketing",
  "salesforce_account_id": "001700000195XqVAAU",
  "play_type": "dark_account",
  "risk_color": "Green",
  "primary_owner": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cc_owners": [],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C08GTNU71R9",
  "renewal_opportunity_id": "006Po00000mjpdrIAA",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000mjpdrIAA/view",
  "drivers": [
    "Renewal date: 2028-07-30",
    "ARR: $54,170",
    "ATR at risk: $27,085",
    "CSE sentiment commentary last updated 150d ago",
    "No dedicated CSE (digital coverage)",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 3 min (30d) — below threshold"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Missional Marketing Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [
    "No CSE assigned (digital coverage)"
  ],
  "cse_sentiment_commentary": "<p></p><p color=\"\" style=\"\">Multi year renewal done.</p><p color=\"\" style=\"\">Contract is secure via a 36‑month renewal signed in July 2025, aligning the next renewal to July 2028 and indicating commercial stability</p><p color=\"\" style=\"\">Not much growth opportunity . we will continue to engage with Kevin, CEO to see how best Zuora can support them</p>",
  "commentary_last_updated": "2026-01-26T16:38:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cse": null,
  "priority_score": 92,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "CSE sentiment commentary last updated 150d ago",
      "observedAt": "2026-01-26T16:38:00.000+0000"
    },
    {
      "source": "salesforce",
      "signal": "No dedicated CSE (digital coverage)"
    },
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 3 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000mjpdrIAA:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 27084.89,
  "renewal_opportunity_name": "Missional Marketing Billing 07-28"
}
```

---

## CTA 5 — Science News

```json
{
  "cta_id": "expand3-2026-06-25-science-news-dark_account",
  "account_name": "Science News",
  "salesforce_account_id": "0010g00001iSEpPAAW",
  "play_type": "dark_account",
  "risk_color": "Green",
  "primary_owner": {
    "name": "Christopher Franklin-Hollier",
    "role": "CSE"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0AE2FELL2G",
  "renewal_opportunity_id": "006Po00000SqOSjIAN",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000SqOSjIAN/view",
  "drivers": [
    "Renewal date: 2027-12-14",
    "ARR: $154,224",
    "ATR at risk: $77,112",
    "CSE sentiment commentary last updated 230d ago",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 1 min (30d) — below threshold"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Science News Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p>The customer had a few issues that were solved. <br><br>- B2B account login was failing due to a misconfiguration.<br>- Login/Email form validation is now working and they will apply it in Production. </p>",
  "commentary_last_updated": "2025-11-07T14:54:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Christopher Franklin-Hollier",
    "role": "CSE"
  },
  "priority_score": 86,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "CSE sentiment commentary last updated 230d ago",
      "observedAt": "2025-11-07T14:54:00.000+0000"
    },
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 1 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000SqOSjIAN:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 77112,
  "renewal_opportunity_name": "Science News Zephr 12/2027"
}
```

---

## CTA 6 — Focus-N-Fly, Inc.

```json
{
  "cta_id": "expand3-2026-06-25-focus-n-fly--inc--dark_account",
  "account_name": "Focus-N-Fly, Inc.",
  "salesforce_account_id": "0017000000TZ5noAAD",
  "play_type": "dark_account",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cc_owners": [],
  "destination_slack_channel": "https://app.slack.com/client/E03RP62KP5H/C0B2ZRFL6LW",
  "renewal_opportunity_id": "006Po00000Zio3RIAR",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000Zio3RIAR/view",
  "drivers": [
    "Renewal date: 2028-02-29",
    "ARR: $14,520",
    "ATR at risk: $7,260",
    "No dedicated CSE (digital coverage)",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 1 min (30d) — below threshold",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Focus-N-Fly, Inc. Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [
    "No CSE assigned (digital coverage)"
  ],
  "cse_sentiment_commentary": "<p></p><div class=\"gs-section-title\" color=\"\" style=\"\"><h6 class=\"ant-typography ant-typography-title-section\" style=\"\">Note</h6></div><div class=\"content-section\" color=\"\" style=\"\"><div class=\"content-holder\" style=\"\"><div class=\"content\" style=\"\"><p style=\"\"></p><p color=\"\" style=\"\">State &amp; Renewal Risk:<br style=\"\">Customer renewed and contract till 2028,though year-over-year metrics show limited growth. The current lack of regular interaction poses a potential renewal risk.</p><p color=\"\" style=\"\">Account Plan:</p><p color=\"\" style=\"\">Follow ups are <a href=\"http://done.no/\" target=\"_blank\" color=\"\" style=\"\">done.No</a> response from customer</p></div></div></div>",
  "commentary_last_updated": "2026-05-22T02:34:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cse": null,
  "priority_score": 85,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No dedicated CSE (digital coverage)"
    },
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 1 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000Zio3RIAR:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 7260,
  "renewal_opportunity_name": "Focus-N-Fly, Inc. Billing 02-28"
}
```

---

## CTA 7 — Underline Technologies, LLC

```json
{
  "cta_id": "expand3-2026-06-25-underline-technologies--llc-dark_account",
  "account_name": "Underline Technologies, LLC",
  "salesforce_account_id": "001Po00000CIlpZIAT",
  "play_type": "dark_account",
  "risk_color": "Green",
  "primary_owner": {
    "name": "Dominic Varner",
    "role": "AE"
  },
  "cc_owners": [],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C091Q3P9MV4",
  "renewal_opportunity_id": "006Po00000HVCeUIAX",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000HVCeUIAX/view",
  "drivers": [
    "Renewal date: 2027-06-28",
    "ARR: $152,000",
    "ATR at risk: $76,000",
    "CSE sentiment commentary last updated 167d ago",
    "No dedicated CSE (digital coverage)",
    "No workshop logged in the last 365 days"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Underline Technologies, LLC Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [
    "No CSE assigned (digital coverage)"
  ],
  "cse_sentiment_commentary": "<ul color=\"\" style=\"\"><li style=\"\">STATE AND RENEWAL RISK: KR : Recent go live on Nov-2024. Another Estuate implementation ans working with them to get additonal details and product setup.<p color=\"\" style=\"\"><br style=\"\"></p><p color=\"\" style=\"\">No risk and keeping the account in green.</p></li></ul><ul color=\"\" style=\"\"><li style=\"\">Next steps : No response from the customer on NetSuite connector. Follow up and keep them engaged.</li></ul>",
  "commentary_last_updated": "2026-01-09T18:49:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Dominic Varner",
    "role": "AE"
  },
  "cse": null,
  "priority_score": 80,
  "confidence": "low",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "CSE sentiment commentary last updated 167d ago",
      "observedAt": "2026-01-09T18:49:00.000+0000"
    },
    {
      "source": "salesforce",
      "signal": "No dedicated CSE (digital coverage)"
    },
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    }
  ],
  "dedup_key": "006Po00000HVCeUIAX:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 76000,
  "renewal_opportunity_name": "Underline Technologies, LLC Zuora  Renewal June 2027"
}
```

---

## CTA 8 — Omni Technology Solutions Inc

```json
{
  "cta_id": "expand3-2026-06-25-omni-technology-solutions-inc-dark_account",
  "account_name": "Omni Technology Solutions Inc",
  "salesforce_account_id": "0017000000j2pYoAAI",
  "play_type": "dark_account",
  "risk_color": "Red",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B35RPBCJY",
  "renewal_opportunity_id": "006Po00000zo7nxIAA",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000zo7nxIAA/view",
  "drivers": [
    "Renewal date: 2026-11-29",
    "ARR: $176,208",
    "ATR at risk: $88,104",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 2 min (30d) — below threshold",
    "CSE Sentiment: Red"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Omni Technology Solutions Inc Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><div class=\"gs-section-title\" color=\"\" style=\"\"><h6 class=\"ant-typography ant-typography-title-section\" style=\"\">Note</h6></div><div class=\"content-section\" color=\"\" style=\"\"><div class=\"content-holder\" style=\"\"><div class=\"content\" style=\"\"><p style=\"\"></p><p color=\"\" style=\"\">RENEWAL STATE (AMBER):  declining utilization signals potential business slowdown and risk of future volume restructuring. </p><p color=\"\" style=\"\">ACCOUNT PLAN: Conduct executive alignment/reset to reconfirm strategic goals and roadmap, closely monitor utilization trends, and proactively scenario-plan for volume adjustments to protect renewal stability and long-term growth.</p></div></div></div>",
  "commentary_last_updated": "2026-06-22T06:57:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 77,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 2 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000zo7nxIAA:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 88103.8,
  "renewal_opportunity_name": "Omni Technology Solutions Inc Billing 11-26"
}
```

---

## CTA 9 — NorthStar Travel Media, LLC

```json
{
  "cta_id": "expand3-2026-06-25-northstar-travel-media--llc-dark_account",
  "account_name": "NorthStar Travel Media, LLC",
  "salesforce_account_id": "0017000000dJGzAAAW",
  "play_type": "dark_account",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B2LPJC1PZ",
  "renewal_opportunity_id": "006Po00000tUTrKIAW",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000tUTrKIAW/view",
  "drivers": [
    "Renewal date: 2026-09-30",
    "ARR: $96,419",
    "ATR at risk: $48,210",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 1 min (30d) — below threshold",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "NorthStar Travel Media, LLC Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><ul><li class=\"p8i6j0a\"><p class=\"p8i6j01 paragraph\"><b>State &amp; renewal:</b> Renewal risk.<span style=\"font-family: Roboto, -apple-system, BlinkMacSystemFont,;\" color=\"\">YoY metrics are not great and the main watchout is low engagement/low usage </span></p></li><li class=\"p8i6j0a\"><p class=\"p8i6j01 paragraph\"><b>Account plan:</b> Re-establish a monthly cadence with the customer, continue the NetSuite connector discussion, and use the renewal conversation to improve engagement and confirm support needs/value realization. </p></li></ul>",
  "commentary_last_updated": "2026-06-22T06:35:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 77,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 1 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000tUTrKIAW:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 48209.7,
  "renewal_opportunity_name": "NorthStar Travel Media, LLC Billing 09-26"
}
```

---

## CTA 10 — inContact

```json
{
  "cta_id": "expand3-2026-06-25-incontact-dark_account",
  "account_name": "inContact",
  "salesforce_account_id": "0010g00001aLDKsAAO",
  "play_type": "dark_account",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Ethan Wookey",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C08DMLZ65PZ",
  "renewal_opportunity_id": "006Po00001GaOWjIAN",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00001GaOWjIAN/view",
  "drivers": [
    "Renewal date: 2027-04-29",
    "ARR: $260,425",
    "ATR at risk: $130,213",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 0 min (30d) — below threshold",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "inContact Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p color=\"\" style=\"\">State and Renewal</p><p color=\"\" style=\"\">No renewal risk.Customer signed for next 1 year.YOY metrics are declining.</p><p color=\"\" style=\"\">Account Plan:</p><p color=\"\" style=\"\">Folow up with the customer to set cadence call and continue the engagement.</p>",
  "commentary_last_updated": "2026-06-22T06:37:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 76,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 0 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00001GaOWjIAN:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 130212.6,
  "renewal_opportunity_name": "inContact Billing 04-27"
}
```

---

## CTA 11 — Elevat Inc.

```json
{
  "cta_id": "expand3-2026-06-25-elevat-inc--dark_account",
  "account_name": "Elevat Inc.",
  "salesforce_account_id": "0017000001UwQFMAA3",
  "play_type": "dark_account",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B2LNNGY5V",
  "renewal_opportunity_id": "006Po000018p8JOIAY",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po000018p8JOIAY/view",
  "drivers": [
    "Renewal date: 2027-02-13",
    "ARR: $66,625",
    "ATR at risk: $33,313",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 5 min (30d) — below threshold",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Elevat Inc. Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p class=\"\" color=\"\" style=\"\">State and Renewal<br style=\"\">Renewal done for next 1 year with 1m in Q1(Auto renewed)</p><p class=\"\" color=\"\" style=\"\">No major concerns.</p><p class=\"\" color=\"\" style=\"\">2. Account Plan<br style=\"\">Account review done and following with the customer to have monthly cadence.Customer asked for the recent contract to be reviewed.</p>",
  "commentary_last_updated": "2026-06-22T06:38:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 76,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 5 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po000018p8JOIAY:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 33312.52,
  "renewal_opportunity_name": "Elevat Inc. Billing 02-27"
}
```

---

## CTA 12 — Data Processing Design Inc

```json
{
  "cta_id": "expand3-2026-06-25-data-processing-design-inc-dark_account",
  "account_name": "Data Processing Design Inc",
  "salesforce_account_id": "0014u00001twlSRAAY",
  "play_type": "dark_account",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Ethan Wookey",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://app.slack.com/client/E03RP62KP5H/C0B315C8AER",
  "renewal_opportunity_id": "006Po00000CqfC5IAJ",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000CqfC5IAJ/view",
  "drivers": [
    "Renewal date: 2027-05-09",
    "ARR: $125,861",
    "ATR at risk: $62,930",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 3 min (30d) — below threshold",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Data Processing Design Inc Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p color=\"\" style=\"\">State and Renewal :</p><p color=\"\" style=\"\">Customer renewal done for 1 <a href=\"http://year.No\" target=\"_blank\"><span>year.</span></a>There are no current issues reported. Utilisation and YOY metrics are <a href=\"http://good.no/\" target=\"_blank\" color=\"\" style=\"\">good</a>. </p><p color=\"\" style=\"\">Account Plan :</p><p color=\"\" style=\"\">Follw ups are done to be in touch with the cutsomer</p>",
  "commentary_last_updated": "2026-06-22T06:31:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 76,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 3 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000CqfC5IAJ:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 62930.41,
  "renewal_opportunity_name": "Data Processing Design Inc Zuora Renewal May 2027"
}
```

---

## CTA 13 — Dor Technologies

```json
{
  "cta_id": "expand3-2026-06-25-dor-technologies-dark_account",
  "account_name": "Dor Technologies",
  "salesforce_account_id": "0010g00001b8X5cAAE",
  "play_type": "dark_account",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B3266ST8S",
  "renewal_opportunity_id": "006Po00001K9sI5IAJ",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00001K9sI5IAJ/view",
  "drivers": [
    "Renewal date: 2027-05-30",
    "ARR: $118,932",
    "ATR at risk: $59,466",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 1 min (30d) — below threshold",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Dor Technologies Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p>State and renewal:</p><p>Customer renewed for 1 year for 2M.</p><p>Account Plan:</p><p>Low touch <a href=\"http://customer.No\" target=\"_blank\"><span>customer.No</span></a> response from the customers.Continuous follow up has been done.</p>",
  "commentary_last_updated": "2026-06-22T06:27:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 76,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 1 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00001K9sI5IAJ:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 59465.76,
  "renewal_opportunity_name": "Dor Technologies Billing 05-27"
}
```

---

## CTA 14 — Readdle

```json
{
  "cta_id": "expand3-2026-06-25-readdle-dark_account",
  "account_name": "Readdle",
  "salesforce_account_id": "00170000014fI1lAAE",
  "play_type": "dark_account",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B3287NP42",
  "renewal_opportunity_id": "006Po00000zgflCIAQ",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000zgflCIAQ/view",
  "drivers": [
    "Renewal date: 2026-11-29",
    "ARR: $127,427",
    "ATR at risk: $63,714",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 3 min (30d) — below threshold",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Readdle Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p class=\"\" color=\"\" style=\"\">State and Renewal<br style=\"\">No engagement with customer, leads to renewal risk.YOY metrics are not great.Utilisation is OK.</p><p class=\"\" color=\"\" style=\"\">Account Plan</p><p class=\"\" color=\"\" style=\"\">Customer refused to have the account review.Trying to set cadence call</p>",
  "commentary_last_updated": "2026-06-22T06:37:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 76,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 3 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000zgflCIAQ:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 63713.51,
  "renewal_opportunity_name": "Readdle Billing 11-26"
}
```

---

## CTA 15 — Enverus Inc. (fka Cortex Business Solutions)

```json
{
  "cta_id": "expand3-2026-06-25-enverus-inc---fka-cortex-business-solutions--dark_account",
  "account_name": "Enverus Inc. (fka Cortex Business Solutions)",
  "salesforce_account_id": "0017000000klypeAAA",
  "play_type": "dark_account",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B304Y6S6A",
  "renewal_opportunity_id": "006Po00001879RGIAY",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00001879RGIAY/view",
  "drivers": [
    "Renewal date: 2027-02-13",
    "ARR: $138,280",
    "ATR at risk: $69,140",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 2 min (30d) — below threshold",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Enverus Inc. (fka Cortex Business Solutions) Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p color=\"\" style=\"\">State and Renewal:<br style=\"\">Renewal done for next 1 year with 3M in Q1</p><p color=\"\" style=\"\">Account Plan:</p><p color=\"\" style=\"\">Following with customer to have cadence call.But currently customer declined and they dont have major concerns</p>",
  "commentary_last_updated": "2026-06-22T06:41:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 76,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 2 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00001879RGIAY:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 69140.08,
  "renewal_opportunity_name": "Enverus Inc. (fka Cortex Business Solutions) Billing 02-27"
}
```

---

## CTA 16 — Arista Networks, Inc.

```json
{
  "cta_id": "expand3-2026-06-25-arista-networks--inc--managed_wind_down",
  "account_name": "Arista Networks, Inc.",
  "salesforce_account_id": "0017000000WGwFUAA1",
  "play_type": "managed_wind_down",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Robin Saxena",
    "role": "CSE"
  },
  "cc_owners": [
    {
      "name": "Jeanie Isenhour",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C08GQ1X1XUM",
  "renewal_opportunity_id": "006Po00000tF0jCIAS",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000tF0jCIAS/view",
  "drivers": [
    "Renewal date: 2026-09-29",
    "ARR: $273,642",
    "ATR at risk: $136,821",
    "Commentary documents customer wind-down or exit",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Manage wind-down timeline and ensure clean exit.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Arista Networks, Inc. Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><ul><li><b>STATE AND RENEWAL RISK:</b> Arista committed to renewing for at least 2 years despite utilization dropping to 70% due to their Untangle product EOL. AE is pushing for a 3-year term instead. The declining utilization will continue as the EOL product phases out, making the expansion conversation critical to offsetting revenue risk and growing the account.</li></ul><p></p><p><br></p><ul><li><b>ACTION PLAN</b> Scheduling a QTC discovery meeting to identify additional use cases for platform expansion, targeting very low current share of wallet and minimal Orders/Invoice Settlement adoption.</li></ul>",
  "commentary_last_updated": "2026-06-22T10:32:00.000+0000",
  "team_aware": true,
  "ae": {
    "name": "Jeanie Isenhour",
    "role": "AE"
  },
  "cse": {
    "name": "Robin Saxena",
    "role": "CSE"
  },
  "priority_score": 75,
  "confidence": "high",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "Wind-down documented in commentary"
    }
  ],
  "dedup_key": "006Po00000tF0jCIAS:managed_wind_down",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 136821.08,
  "renewal_opportunity_name": "Arista Networks, Inc. Billing 09-26"
}
```

---

## CTA 17 — BI Incorporated

```json
{
  "cta_id": "expand3-2026-06-25-bi-incorporated-dark_account",
  "account_name": "BI Incorporated",
  "salesforce_account_id": "0017000000ZJsEVAA1",
  "play_type": "dark_account",
  "risk_color": "Green",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Ethan Wookey",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C08SVHN0M8D",
  "renewal_opportunity_id": "006Po00000yIDNeIAO",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000yIDNeIAO/view",
  "drivers": [
    "Renewal date: 2026-10-31",
    "ARR: $263,738",
    "ATR at risk: $131,869",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 3 min (30d) — below threshold",
    "CSE Sentiment: Green"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "BI Incorporated Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><div class=\"gs-section-title\" color=\"\" style=\"\"><h6 class=\"ant-typography ant-typography-title-section\" style=\"\"></h6><p></p><ul><li class=\"p8i6j0a\"><p class=\"p8i6j01 paragraph\"><b>State &amp; renewal:</b>  YOY metrics and utilisation are good, but the account needs re-engagement and there is some value/cost sensitivity in the background. </p><div class=\"tk0j8o1 _1ibi0s31a _1ibi0s3en\"><a class=\"wdbi343 wdbi341 _1ibi0s3ec _1ibi0s376\" id=\"base-ui-:r337:\" href=\"https://zuora.lightning.force.com/lightning/r/Account/0017000000ZJsEVAA1/view\" rel=\"noreferrer\" target=\"_blank\"></a><a class=\"wdbi343 wdbi341 _1ibi0s3ec _1ibi0s376\" id=\"base-ui-:r339:\" href=\"https://zuora.my.salesforce.com/servlet/servlet.FileDownload?file=00PPo00000zdZFsMAM\" rel=\"noreferrer\" target=\"_blank\"></a></div></li><li class=\"p8i6j0a\"><p class=\"p8i6j01 paragraph\"><b>Account plan:</b> Re-establish a regular cadence, align on their invoice delivery and development pain points, and use upcoming roadmap / onsite workshop discussions to rebuild engagement with the new finance contacts.</p></li></ul></div>",
  "commentary_last_updated": "2026-06-22T07:07:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 75,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 3 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000yIDNeIAO:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 131868.78,
  "renewal_opportunity_name": "BI Incorporated Billing 10-26"
}
```

---

## CTA 18 — Alchemy Systems

```json
{
  "cta_id": "expand3-2026-06-25-alchemy-systems-dark_account",
  "account_name": "Alchemy Systems",
  "salesforce_account_id": "0017000000pm9KJAAY",
  "play_type": "dark_account",
  "risk_color": "Red",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C08SE81CCV7",
  "renewal_opportunity_id": "006Po00000GczvhIAB",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000GczvhIAB/view",
  "drivers": [
    "Renewal date: 2027-07-31",
    "ARR: $146,454",
    "ATR at risk: $73,227",
    "No workshop logged in the last 365 days",
    "Cerebro engagement risk flagged",
    "Engagio engagement 9 min (30d) — below threshold",
    "CSE Sentiment: Red"
  ],
  "requested_action": "Investigate account status and re-engage.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Alchemy Systems Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p color=\"\" style=\"\">State and renewal :</p><p color=\"\" style=\"\">Renewal is in risk.The client is planning to **migrate to Shopify for e-commerce and transactional processing in 2026** as part of a broader technology consolidation and cost reduction initiative.</p><p color=\"\" style=\"\"><br style=\"\"></p><p color=\"\" style=\"\">Account Plan :</p><p color=\"\" style=\"\">Both parties agreed to maintain communication, with Zuora planning to check in monthly via email to ensure support needs are met.</p>",
  "commentary_last_updated": "2026-06-22T07:00:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 74,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "No workshop logged in the last 365 days"
    },
    {
      "source": "cerebro",
      "signal": "Cerebro engagement risk flagged"
    },
    {
      "source": "salesforce",
      "signal": "Engagio engagement 9 min (30d) — below threshold"
    }
  ],
  "dedup_key": "006Po00000GczvhIAB:dark_account",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 73227,
  "renewal_opportunity_name": "Alchemy Systems Zuora Renewal July 2027"
}
```

---

## CTA 19 — Compulink Management Center, Inc. dba Laserfiche

```json
{
  "cta_id": "expand3-2026-06-25-compulink-management-center--inc--dba-laserfiche-managed_wind_down",
  "account_name": "Compulink Management Center, Inc. dba Laserfiche",
  "salesforce_account_id": "0017000000WlUMmAAN",
  "play_type": "managed_wind_down",
  "risk_color": "Green",
  "primary_owner": {
    "name": "Kyle Larkin",
    "role": "CSE"
  },
  "cc_owners": [
    {
      "name": "Jeanie Isenhour",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C07G517QV2A",
  "renewal_opportunity_id": "006Po00000hiHijIAE",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000hiHijIAE/view",
  "drivers": [
    "Renewal date: 2028-06-14",
    "ARR: $581,550",
    "ATR at risk: $290,775",
    "Commentary documents customer wind-down or exit",
    "CSE Sentiment: Green"
  ],
  "requested_action": "Manage wind-down timeline and ensure clean exit.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Compulink Management Center, Inc. dba Laserfiche Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<div><b>STATE AND RENEWAL RISK:</b> The account is active with high engagement but sustained low implementation health — the Zuora CPQ-X (CPQX) GS implementation has been reported as Overall Health: Red for five consecutive weeks (May 1 through June 5, 2026), formal UAT has not yet commenced due to unresolved requirements, defects, and scope disputes, and the Staircase AI Health Score sits at 56/100 against an Engagement Score of 80. </div><div>The renewal opportunity itself reflects a positive ACV uplift with no downsell identified, and a recent onsite visit by the Zuora team on May 27, 2026 demonstrates continued executive commitment, but the persistent implementation friction — including an unresolved Salesforce Shield validation dependency outstanding since January 2025, active scope disputes around prospect/reseller quoting logic (CO2), and delayed UAT — represent meaningful risks to platform stickiness and customer sentiment if not resolved promptly. </div><div><br></div><div><b>ACTION PLAN</b>: The immediate priority is participating in Laserfiche&#39;s requested June alignment and demo sessions (week of June 8) to confirm UAT entry criteria, establish a joint UAT burn-down cadence with clear defect ownership, and deliver a written Zuora CPQ + Salesforce Shield compatibility brief by June 12, 2026 — an item that has been outstanding since the previous sentiment cycle. </div><div>In parallel, Jeanie Isenhour (AE) and Kyle Larkin (CSE) will coordinate an executive alignment call with Laserfiche&#39;s CIO and/or CFO by June 20, 2026 to confirm CPQ direction and address implementation concerns, while GS leads Leena Johnson and Girish Kumar drive weekly implementation status and a bi-weekly internal SteerCo with GS leadership (Andrew Vaz) to track risks and exit criteria.</div>",
  "commentary_last_updated": "2026-06-06T17:11:00.000+0000",
  "team_aware": true,
  "ae": {
    "name": "Jeanie Isenhour",
    "role": "AE"
  },
  "cse": {
    "name": "Kyle Larkin",
    "role": "CSE"
  },
  "priority_score": 74,
  "confidence": "high",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "Wind-down documented in commentary"
    }
  ],
  "dedup_key": "006Po00000hiHijIAE:managed_wind_down",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 290775.22,
  "renewal_opportunity_name": "Compulink Management Center, Inc. dba Laserfiche Billing 06-28"
}
```

---

## CTA 20 — Elm Street Technology, LLC

```json
{
  "cta_id": "expand3-2026-06-25-elm-street-technology--llc-managed_wind_down",
  "account_name": "Elm Street Technology, LLC",
  "salesforce_account_id": "0014u00001pxMOjAAM",
  "play_type": "managed_wind_down",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Kyle Larkin",
    "role": "CSE"
  },
  "cc_owners": [
    {
      "name": "Tyler Villaroman",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C086JUNBAJC",
  "renewal_opportunity_id": "006Po00001FsRlFIAV",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00001FsRlFIAV/view",
  "drivers": [
    "Renewal date: 2027-04-23",
    "ARR: $994,431",
    "ATR at risk: $497,215",
    "Commentary documents customer wind-down or exit",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Manage wind-down timeline and ensure clean exit.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Elm Street Technology, LLC Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p> <b>STATE AND RENEWAL RISK:</b> The FY28 renewal (close date April 23, 2027) remains modeled as a full churn/downsell at -$497K ACV driven by overcommitted contract volume and persistent platform underutilization. However, the engagement has materially improved since the last sentiment update: accounting periods have been closed through December 2025 with the Mass Updater template ready to redistribute revenue into historical 2025 periods, <span style=\"font-family: Roboto, -apple-system, BlinkMacSystemFont,;\" color=\"\">and new CFO Eric Amblard — introduced on May 27 — has acknowledged that the implementation failures stem from prior SIs rather than Zuora&#39;s product and is constructively engaged, though he has set hard deadlines of June 30 (new product launch) and July 9 (board meeting) as proof points.</span></p><div class=\"tk0j8o1 _1ibi0s31a _1ibi0s3en\"><a class=\"wdbi343 wdbi341 _1ibi0s3ec _1ibi0s376\" id=\"base-ui-:rcmv:\" href=\"https://zuora.enterprise.slack.com/archives/C086JUNBAJC/p1779905295832549?thread_ts=1779905295.832549&amp;cid=C086JUNBAJC\" rel=\"noreferrer\" target=\"_blank\"></a></div>What is working: the core platform is operationally stable (StaircaseAI Health Score 90), the TAM cadence with Siddhant Verma and Hans Saunders remains active, monthly value reports are being delivered, and the June 9 Tampa onsite workshop — attended by Eric Amblard, Tammy Grant, Hans Saunders, and Bishop Lafer — is the highest-stakes engagement milestone to date with a well-structured agenda covering RevRec remediation, July product launch readiness, Sage Intacct JE automation, and Canadian BU onboarding scoping<p><br></p><p><b>ACTION PLAN:</b> The immediate priority is executing the June 9 Tampa onsite workshop (Kyle Larkin, Nick Wilbur, Amy Costandi leading sessions; Michael Katzman and Tyler Villaroman attending) and delivering a post-workshop recommendation deck by approximately June 13, covering a concrete RevRec roadmap and the July new product launch E2E setup in Zuora — both of which must be demonstrably in motion before Eric Amblard&#39;s July 9 board meeting. </p><div class=\"tk0j8o1 _1ibi0s31a _1ibi0s3en\"><a class=\"wdbi343 wdbi341 _1ibi0s3ec _1ibi0s376\" id=\"base-ui-:rcnb:\" href=\"https://docs.google.com/presentation/d/1rGKGDuAR1dGBz6jaxX1B6vOmc2uJDPQogwGmJmOGjOo\" rel=\"noreferrer\" target=\"_blank\"></a><a class=\"wdbi343 wdbi341 _1ibi0s3ec _1ibi0s376\" id=\"base-ui-:rcne:\" href=\"https://zuora.gainsightcloud.com/v1/ui/timeline#/activities/1I004SG7RDV06L1HFJ7J08YHC7RTQHERB724\" rel=\"noreferrer\" target=\"_blank\"></a></div><p>In parallel, Michael Katzman will stand up bi-weekly executive syncs with Eric, Hans, and Tammy, while Tyler Villaroman, Nick Wilbur, and Ronak Majmudar begin defining the 3-year commercial structure (right-sized volume + RevRec bundle) with the goal of converting the FY28 renewal to a multi-year commitment before the customer&#39;s exit window closes.</p>",
  "commentary_last_updated": "2026-06-06T16:35:00.000+0000",
  "team_aware": true,
  "ae": {
    "name": "Tyler Villaroman",
    "role": "AE"
  },
  "cse": {
    "name": "Kyle Larkin",
    "role": "CSE"
  },
  "priority_score": 73,
  "confidence": "high",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "Wind-down documented in commentary"
    }
  ],
  "dedup_key": "006Po00001FsRlFIAV:managed_wind_down",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 497215.4,
  "renewal_opportunity_name": "Elm Street Technology, LLC Billing 04-27"
}
```

---

## CTA 21 — Devex

```json
{
  "cta_id": "expand3-2026-06-25-devex-dark_renewal",
  "account_name": "Devex",
  "salesforce_account_id": "0017000000mNirEAAS",
  "play_type": "dark_renewal",
  "risk_color": "Red",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C091FRKT95M",
  "renewal_opportunity_id": "006Po00000mT1rUIAS",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000mT1rUIAS/view",
  "drivers": [
    "Renewal date: 2026-07-21",
    "ARR: $138,000",
    "ATR at risk: $69,000",
    "Renewal opp lacks recent next steps",
    "CSE Sentiment: Red"
  ],
  "requested_action": "Re-engage ahead of upcoming renewal.",
  "deadline": "2026-07-14",
  "check_back_date": "2026-07-07",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-07",
    "auto_check_query": "Devex Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-14",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><ul><li class=\"p8i6j0a\"><p class=\"p8i6j01 paragraph\"><b>State &amp; renewal:</b> Devex is currently a red/high-risk renewal account; the customer wants flexibility, is evaluating alternatives, and the current proposal on the table is a 1-year renewal through 7/21/2027  billing with no auto-renewal. </p><div class=\"tk0j8o1 _1ibi0s31a _1ibi0s3en\"><a class=\"wdbi343 wdbi341 _1ibi0s3ec _1ibi0s376\" id=\"base-ui-:r2m2:\" href=\"https://zuora.lightning.force.com/lightning/r/Account/0017000000mNirEAAS/view\" rel=\"noreferrer\" target=\"_blank\"></a><a class=\"wdbi343 wdbi341 _1ibi0s3ec _1ibi0s376\" id=\"base-ui-:r2m4:\" href=\"https://zuora.lightning.force.com/lightning/r/ContentDocument/069Po00001dlUzpIAE/view\" rel=\"noreferrer\" target=\"_blank\"></a></div></li><li class=\"p8i6j0a\"><p class=\"p8i6j01 paragraph\"><b>Account plan:</b> Maintain a regular check-in cadence, focus on fixing the analytics/dashboard and reporting pain points, and use TAM/product review support to simplify usage/reporting and reinforce value before renewal closes</p></li></ul>",
  "commentary_last_updated": "2026-06-22T06:25:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 72,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "Renewal opp stage: 3.0 Define"
    }
  ],
  "dedup_key": "006Po00000mT1rUIAS:dark_renewal",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 69000,
  "renewal_opportunity_name": "Devex Billing 07-26"
}
```

---

## CTA 22 — Leafly, LLC

```json
{
  "cta_id": "expand3-2026-06-25-leafly--llc-dark_renewal",
  "account_name": "Leafly, LLC",
  "salesforce_account_id": "0017000000xKYBZAA4",
  "play_type": "dark_renewal",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C03ULQFE6V6",
  "renewal_opportunity_id": "006Po00000nHbLXIA0",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000nHbLXIA0/view",
  "drivers": [
    "Renewal date: 2026-07-29",
    "ARR: $577,046",
    "ATR at risk: $288,523",
    "Renewal opp lacks recent next steps",
    "CSE Sentiment: Red"
  ],
  "requested_action": "Re-engage ahead of upcoming renewal.",
  "deadline": "2026-07-16",
  "check_back_date": "2026-07-09",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-09",
    "auto_check_query": "Leafly, LLC Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-16",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p color=\"\" style=\"\">State and renewa risk:</p><p color=\"\" style=\"\">Renewal risk is moderate and tied to cost sensitivity—ROI and automation value must be clearly demonstrated.</p><p color=\"\" style=\"\">Customer is frustrated with manual reconciliation and fragmented reporting, exploring Zuora Revenue but still early in evaluation.</p><p color=\"\" style=\"\">Account Plan</p><p color=\"\" style=\"\">Position Zuora Revenue as the automation and consolidation solution to eliminate manual JEs and improve forecasting visibility.</p>",
  "commentary_last_updated": "2026-06-22T06:57:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 72,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "Renewal opp stage: 3.0 Define"
    }
  ],
  "dedup_key": "006Po00000nHbLXIA0:dark_renewal",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 288523.2,
  "renewal_opportunity_name": "Leafly, LLC Billing 07-26"
}
```

---

## CTA 23 — Inmar, Inc.

```json
{
  "cta_id": "expand3-2026-06-25-inmar--inc--utilization_risk",
  "account_name": "Inmar, Inc.",
  "salesforce_account_id": "0017000000rFyeMAAS",
  "play_type": "utilization_risk",
  "risk_color": "Red",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Brandon LaTourelle",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C07NEU5NY6B",
  "renewal_opportunity_id": "006Po00000tPfedIAC",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000tPfedIAC/view",
  "drivers": [
    "Renewal date: 2026-09-30",
    "ARR: $157,500",
    "ATR at risk: $157,500",
    "Cerebro utilization risk flagged",
    "CSE Sentiment: Red"
  ],
  "requested_action": "Investigate usage patterns and build remediation plan.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Inmar, Inc. Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><div class=\"gs-section-title\" style=\"\" color=\"\"><h6 class=\"ant-typography ant-typography-title-section\" style=\"\">Note</h6></div><div class=\"content-section\" style=\"\" color=\"\"><div class=\"content-holder\" style=\"\"><div class=\"content\" style=\"\"><p style=\"\"></p><p color=\"\" style=\"\">State and Renewal:<br style=\"\">The customer has stopped responding, is not attending cadence calls, and declined the workshop, which puts the upcoming renewal at risk.</p><p color=\"\" style=\"\">Account Plan:<br style=\"\">We have reached out to the customer to address their technical issues with PM support and are currently awaiting their response.</p></div></div></div>",
  "commentary_last_updated": "2026-06-22T04:31:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Brandon LaTourelle",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 67,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "cerebro",
      "signal": "Utilization risk"
    }
  ],
  "dedup_key": "006Po00000tPfedIAC:utilization_risk",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 157500,
  "renewal_opportunity_name": "Inmar, Inc. Revenue 09-26"
}
```

---

## CTA 24 — RPost, Inc.

```json
{
  "cta_id": "expand3-2026-06-25-rpost--inc--utilization_risk",
  "account_name": "RPost, Inc.",
  "salesforce_account_id": "0017000000kmr2xAAA",
  "play_type": "utilization_risk",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Ethan Wookey",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B3WB4PMCY",
  "renewal_opportunity_id": "006Po00000EIQOzIAP",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000EIQOzIAP/view",
  "drivers": [
    "Renewal date: 2027-05-31",
    "ARR: $168,561",
    "ATR at risk: $84,281",
    "Cerebro utilization risk flagged",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Investigate usage patterns and build remediation plan.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "RPost, Inc. Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p class=\"\" color=\"\" style=\"\">State and Renewal<br style=\"\">Year-over-year performance and utilization metrics are good,But no regular cadence or engagement with customer.Due to lack of engagement the account is in amber.</p><p class=\"\" color=\"\" style=\"\">Account Plan</p><p class=\"\" color=\"\" style=\"\">Customer engagement is on and off for usecsaes.Trying to establish account review.Dec Follow up sent</p>",
  "commentary_last_updated": "2026-06-22T06:41:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 65,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "cerebro",
      "signal": "Utilization risk"
    }
  ],
  "dedup_key": "006Po00000EIQOzIAP:utilization_risk",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 84280.72,
  "renewal_opportunity_name": "RPost, Inc. Zuora Renewal May 2027"
}
```

---

## CTA 25 — Editshare, LLC

```json
{
  "cta_id": "expand3-2026-06-25-editshare--llc-dark_renewal",
  "account_name": "Editshare, LLC",
  "salesforce_account_id": "0017000000kkYUeAAM",
  "play_type": "dark_renewal",
  "risk_color": "Red",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C0B304Q2TC6",
  "renewal_opportunity_id": "006Po00000rCWf8IAG",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000rCWf8IAG/view",
  "drivers": [
    "Renewal date: 2027-09-29",
    "ARR: $208,000",
    "ATR at risk: $104,000",
    "Renewal opp lacks recent next steps",
    "CSE Sentiment: Red"
  ],
  "requested_action": "Re-engage ahead of upcoming renewal.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Editshare, LLC Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p color=\"\" style=\"\">State and Renewal :</p><p color=\"\" style=\"\"><br style=\"\"></p><p color=\"\" style=\"\">Renewal is at risk.Last year Renewal – Downsell with Churn Risk = “At Risk”, even though it closed On Time (Closed/Won) and extends the contract to 30 Sep 2027.</p><p color=\"\" style=\"\">YOY Metrics are not good.Utilisation is good.</p><p color=\"\" style=\"\"><br style=\"\"></p><p color=\"\" style=\"\">Account Plan:</p><div class=\"tk0j8o1 _1ibi0s31a _1ibi0s3dn\" color=\"\" style=\"\"><a class=\"wdbi343 wdbi341 _1ibi0s3dc _1ibi0s376\" id=\"base-ui-:r31a:\" href=\"https://zuora.lightning.force.com/lightning/r/Account/0017000000kkYUeAAM/view\" rel=\"noreferrer\" target=\"_blank\" color=\"\" style=\"\"></a></div><p color=\"\" style=\"\">the customer wants time until Q2 next year to decide how they go forward, so we should treat longer‑term renewal as at risk and keep close engagement.</p>",
  "commentary_last_updated": "2026-06-22T06:43:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 55,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "Renewal opp stage: 1.0 Qualify"
    }
  ],
  "dedup_key": "006Po00000rCWf8IAG:dark_renewal",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 104000,
  "renewal_opportunity_name": "Editshare, LLC Billing 09-27"
}
```

---

## CTA 26 — Venn Inc (fka OS33)

```json
{
  "cta_id": "expand3-2026-06-25-venn-inc--fka-os33--dark_renewal",
  "account_name": "Venn Inc (fka OS33)",
  "salesforce_account_id": "0017000000koD1zAAE",
  "play_type": "dark_renewal",
  "risk_color": "Yellow",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C09BT0Q7FL3",
  "renewal_opportunity_id": "006Po00001GsWlxIAF",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00001GsWlxIAF/view",
  "drivers": [
    "Renewal date: 2027-04-13",
    "ARR: $110,000",
    "ATR at risk: $55,000",
    "Renewal opp lacks recent next steps",
    "CSE Sentiment: Yellow"
  ],
  "requested_action": "Re-engage ahead of upcoming renewal.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Venn Inc (fka OS33) Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p color=\"\" style=\"\">State &amp; Renewal Update:</p><p color=\"\" style=\"\">Customer is going to renew for 1 year and mentioned looking at a couple other solutions to see if they can handle our use cases better.</p><p color=\"\" style=\"\">AE plans to recommend TAM after renewal.</p><p color=\"\" style=\"\">Account Plan:</p><p color=\"\" style=\"\">Monthly cadence has been set,Continuous engagement with customer to support all the queries.</p>",
  "commentary_last_updated": "2026-04-28T10:37:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 55,
  "confidence": "low",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "Renewal opp stage: 1.0 Qualify"
    }
  ],
  "dedup_key": "006Po00001GsWlxIAF:dark_renewal",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 55000,
  "renewal_opportunity_name": "Venn Inc (fka OS33) Billing 04-27"
}
```

---

## CTA 27 — Rimini Street, Inc.

```json
{
  "cta_id": "expand3-2026-06-25-rimini-street--inc--dark_renewal",
  "account_name": "Rimini Street, Inc.",
  "salesforce_account_id": "0017000000nsQV2AAM",
  "play_type": "dark_renewal",
  "risk_color": "Green",
  "primary_owner": {
    "name": "Sriganth Balusamy",
    "role": "CSE"
  },
  "cc_owners": [
    {
      "name": "Brandon LaTourelle",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C08G3JUJ9QA",
  "renewal_opportunity_id": "006Po00000Wo7S7IAJ",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000Wo7S7IAJ/view",
  "drivers": [
    "Renewal date: 2027-01-31",
    "ARR: $290,624",
    "ATR at risk: $290,624",
    "Renewal opp lacks recent next steps",
    "CSE Sentiment: Green"
  ],
  "requested_action": "Re-engage ahead of upcoming renewal.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Rimini Street, Inc. Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p>Green. Expected to renew. AI demo planned for this week. Will reestablish connection with them. Will try to get some insights in to the ERP implementation as well</p>",
  "commentary_last_updated": "2026-05-25T14:39:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Brandon LaTourelle",
    "role": "AE"
  },
  "cse": {
    "name": "Sriganth Balusamy",
    "role": "CSE"
  },
  "priority_score": 55,
  "confidence": "medium",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "Renewal opp stage: 2.0 Discover"
    }
  ],
  "dedup_key": "006Po00000Wo7S7IAJ:dark_renewal",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 290624,
  "renewal_opportunity_name": "Rimini Street, Inc. RevPro 1/2027"
}
```

---

## CTA 28 — InfluxData Inc.

```json
{
  "cta_id": "expand3-2026-06-25-influxdata-inc--dark_renewal",
  "account_name": "InfluxData Inc.",
  "salesforce_account_id": "0017000001TLW8SAAX",
  "play_type": "dark_renewal",
  "risk_color": "Green",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Ethan Wookey",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C08GG9EN4Q4",
  "renewal_opportunity_id": "006Po00001GzSP3IAN",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00001GzSP3IAN/view",
  "drivers": [
    "Renewal date: 2027-04-29",
    "ARR: $152,443",
    "ATR at risk: $76,222",
    "Renewal opp lacks recent next steps",
    "CSE Sentiment: Green"
  ],
  "requested_action": "Re-engage ahead of upcoming renewal.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "InfluxData Inc. Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><div class=\"gs-section-title\" style=\"\" color=\"\"><h6 class=\"ant-typography ant-typography-title-section\" style=\"\">Note</h6></div><div class=\"content-section\" style=\"\" color=\"\"><div class=\"content-holder\" style=\"\"><div class=\"content\" style=\"\"><p style=\"\"></p><p style=\"\">State and renewal.</p><p style=\"\">No renewal risk.signed Influxdata 2 renewal(coming from a 1 year) at a 5% price increase; goals for the near future:</p><p style=\"\">Account Plan</p><ul style=\"\"><li style=\"\">Get their utilization moving in the right direction</li><li style=\"\">Show our Netsuite Connector</li><li style=\"\">Get them using Zuora AI</li></ul></div></div></div>",
  "commentary_last_updated": "2026-06-22T07:02:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Ethan Wookey",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 54,
  "confidence": "low",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "Renewal opp stage: 1.0 Qualify"
    }
  ],
  "dedup_key": "006Po00001GzSP3IAN:dark_renewal",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 76221.62,
  "renewal_opportunity_name": "InfluxData Inc. Billing 04-27"
}
```

---

## CTA 29 — Canopy Tax

```json
{
  "cta_id": "expand3-2026-06-25-canopy-tax-dark_renewal",
  "account_name": "Canopy Tax",
  "salesforce_account_id": "00170000014hec7AAA",
  "play_type": "dark_renewal",
  "risk_color": "Green",
  "primary_owner": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "cc_owners": [
    {
      "name": "Cameron Challoner",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C08UAF7RMGB",
  "renewal_opportunity_id": "006Po00000yUAIjIAO",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000yUAIjIAO/view",
  "drivers": [
    "Renewal date: 2027-11-13",
    "ARR: $390,640",
    "ATR at risk: $195,320",
    "Renewal opp lacks recent next steps",
    "CSE Sentiment: Green"
  ],
  "requested_action": "Re-engage ahead of upcoming renewal.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Canopy Tax Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><div class=\"gs-section-title\" style=\"\" color=\"\"><h6 class=\"ant-typography ant-typography-title-section\" style=\"\">Note</h6></div><div class=\"content-section\" style=\"\" color=\"\"><div class=\"content-holder\" style=\"\"><div class=\"content\" style=\"\"><p style=\"\">No renewal risk.Strong roadmap momentum (CPQX, IS migration, quotes). YOY metrics and utilisation are good.</p><p color=\"\" style=\"\">2. Account Plan<br style=\"\">Regular cadence set with the customer</p></div></div></div>",
  "commentary_last_updated": "2026-06-01T18:32:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Cameron Challoner",
    "role": "AE"
  },
  "cse": {
    "name": "Mahalakshmi S",
    "role": "CSE",
    "slack_handle": "Maha"
  },
  "priority_score": 51,
  "confidence": "low",
  "source_signals": [
    {
      "source": "salesforce",
      "signal": "Renewal opp stage: 1.0 Qualify"
    }
  ],
  "dedup_key": "006Po00000yUAIjIAO:dark_renewal",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 195320,
  "renewal_opportunity_name": "Canopy Tax Billing 11-27"
}
```

---

## CTA 30 — SurePayroll

```json
{
  "cta_id": "expand3-2026-06-25-surepayroll-expertise_risk",
  "account_name": "SurePayroll",
  "salesforce_account_id": "0017000000T09HIAAZ",
  "play_type": "expertise_risk",
  "risk_color": "Green",
  "primary_owner": {
    "name": "Thais Pagliaricci",
    "role": "CSE"
  },
  "cc_owners": [
    {
      "name": "Weston Clegg",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C04SXEFQGKX",
  "renewal_opportunity_id": "006Po000016Liq5IAC",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po000016Liq5IAC/view",
  "drivers": [
    "Renewal date: 2029-01-29",
    "ARR: $1,292,406",
    "ATR at risk: $646,203",
    "Cerebro expertise risk flagged",
    "CSE Sentiment: Green"
  ],
  "requested_action": "Review Cerebro risk signals and build remediation plan.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "SurePayroll Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p></p><p class=\"p8i6j01 paragraph\" style=\"\" color=\"\">STATE AND RENEWAL RISK: No new update other than looking like a new renewal in an attempt to align SurePayroll and Paycor renewals under Paychex parent. </p>",
  "commentary_last_updated": "2026-01-12T15:58:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Weston Clegg",
    "role": "AE"
  },
  "cse": {
    "name": "Thais Pagliaricci",
    "role": "CSE"
  },
  "priority_score": 47,
  "confidence": "high",
  "source_signals": [
    {
      "source": "cerebro",
      "signal": "Cerebro expertise risk flagged"
    }
  ],
  "dedup_key": "006Po000016Liq5IAC:expertise_risk",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 646203,
  "renewal_opportunity_name": "SurePayroll Billing 01-29"
}
```

---

## CTA 31 — Bentley Systems, Incorporated

```json
{
  "cta_id": "expand3-2026-06-25-bentley-systems--incorporated-suite_risk",
  "account_name": "Bentley Systems, Incorporated",
  "salesforce_account_id": "0017000000bEzOcAAK",
  "play_type": "suite_risk",
  "risk_color": "Green",
  "primary_owner": {
    "name": "Sriganth Balusamy",
    "role": "CSE"
  },
  "cc_owners": [
    {
      "name": "Weston Clegg",
      "role": "AE"
    }
  ],
  "destination_slack_channel": "https://zuora.enterprise.slack.com/archives/C08CZ4MGRKR",
  "renewal_opportunity_id": "006Po00000mtYqDIAU",
  "renewal_opportunity_url": "https://zuora.my.salesforce.com/lightning/r/Opportunity/006Po00000mtYqDIAU/view",
  "drivers": [
    "Renewal date: 2028-06-29",
    "ARR: $349,108",
    "ATR at risk: $349,108",
    "Cerebro suite risk flagged",
    "CSE Sentiment: Green"
  ],
  "requested_action": "Review Cerebro risk signals and build remediation plan.",
  "deadline": "2026-07-25",
  "check_back_date": "2026-07-18",
  "expected_artifact": "SFDC update + Slack thread",
  "follow_through": {
    "expected_artifact": "SFDC update + Slack thread",
    "check_back_date": "2026-07-18",
    "auto_check_query": "Bentley Systems, Incorporated Slack OR meeting OR email last 7 days",
    "if_no_response_by": "2026-07-25",
    "then": "Escalate to CSE manager"
  },
  "data_gaps": [],
  "cse_sentiment_commentary": "<p>Rolled out the COA changes in production. yet to hear from them on status of May month close. Follow up sent by CP. Will engage with them in another couple of weeks on AI</p>",
  "commentary_last_updated": "2026-06-11T08:26:00.000+0000",
  "team_aware": false,
  "ae": {
    "name": "Weston Clegg",
    "role": "AE"
  },
  "cse": {
    "name": "Sriganth Balusamy",
    "role": "CSE"
  },
  "priority_score": 46,
  "confidence": "high",
  "source_signals": [
    {
      "source": "cerebro",
      "signal": "Cerebro suite risk flagged"
    }
  ],
  "dedup_key": "006Po00000mtYqDIAU:suite_risk",
  "stale_after": "2026-08-09",
  "atr_at_risk_usd": 349107.7,
  "renewal_opportunity_name": "Bentley Systems, Incorporated Revenue 06-28"
}
```
---

## Accounts evaluated — no CTA

- **Fender Musical Instruments Corporation**: Commentary documents active plan — team aware
- **UPKEEP TECHNOLOGIES, INC**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Perkville (Restart)**: Commentary documents active plan — team aware
- **Brightedge Technologies, Inc.**: Commentary documents active plan — team aware
- **Relativity ODA LLC**: Commentary documents active plan — team aware
- **Brunswick News, a division of Postmedia Network Inc.**: Commentary documents active plan — team aware
- **Appointment Plus**: Open Gainsight CTA covers this play: "Activity Timeline - DaySmart + Zuora Renewal Discussion — Appointment Plus"
- **Yesware, Inc.**: Commentary documents active plan — team aware
- **Rubicon Global**: Commentary documents active plan — team aware
- **Customer Focus Software**: Commentary documents active plan — team aware
- **Teads Holding Co.**: Commentary documents active plan — team aware
- **Zetta, Inc**: Commentary documents active plan — team aware
- **Placester, Inc.**: Open Gainsight CTA covers this play: "Activity Timeline - Zuora/ Placester Renewal Discussion — Placester, Inc."
- **Washington Newspaper Publishing Co, LLC**: Commentary documents active plan — team aware
- **Engageware**: Commentary documents active plan — team aware
- **Perch Energy, LLC**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Wowza Media Systems**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Imagine Communications, Inc.**: Commentary documents active plan — team aware
- **Omnitracs, LLC**: Commentary documents active plan — team aware
- **FuneralOne**: Commentary documents active plan — team aware
- **Sync.com**: Commentary documents active plan — team aware
- **RSA Conference LLC**: Commentary documents active plan — team aware
- **Talkdesk**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Worksuite, LLC**: Commentary documents active plan — team aware
- **Finale, Inc.**: Commentary documents active plan — team aware
- **The San Francisco Standard**: Commentary documents active plan — team aware
- **OVH US LLC**: Commentary documents active plan — team aware
- **Alchemer LLC (fka SurveyGizmo)**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Branch Metrics**: Commentary documents active plan — team aware
- **Tune**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Association of Certified Fraud Examiners**: Commentary documents active plan — team aware
- **Akerna Corp (fka MJ Freeway)**: Open Gainsight CTA covers this play: "Akerna Corp (fka MJ Freeway) Low Utilization"
- **unWired Broadband LLC**: Open Gainsight CTA covers this play: "Activity Timeline - Zuora <> UnWired Renewal  — unWired Broadband LLC"
- **Canon Inc**: Commentary documents active plan — team aware
- **Celartem, Inc.**: Commentary documents active plan — team aware
- **Kintone**: Commentary documents active plan — team aware
- **Digital Air Strike**: Commentary documents active plan — team aware
- **RealNetworks LLC**: Commentary documents active plan — team aware
- **Gaia**: Commentary documents active plan — team aware
- **Act! (fka Swiftpage)**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Prezi**: Commentary documents active plan — team aware
- **The Dun & Bradstreet Corporation**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Guru**: Commentary documents active plan — team aware
- **Dynata, LLC. (fka MarketSight)**: Commentary documents active plan — team aware
- **ReviewTrackers**: Commentary documents active plan — team aware
- **Service Noodle**: Commentary documents active plan — team aware
- **Kustomer, LLC.**: Open Gainsight CTA covers this play: "Activity Timeline - Zuora / Kustomer - Renewal Sync — Kustomer, LLC."
- **Stenograph LLC**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Mobials Inc**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Deloitte LLP**: Commentary documents active plan — team aware
- **Voya Services Company (fka Benefitfocus.com, Inc)**: Customer mid-migration/RFP — paused vendor activity per commentary
- **International Air Transportation Association (IATA)**: Commentary documents active plan — team aware
- **Cricut, Inc.**: Commentary documents active plan — team aware
- **SiteCompli, LLC**: Commentary documents active plan — team aware
- **Adweek, LLC**: Commentary documents active plan — team aware
- **TrackVia**: Commentary documents active plan — team aware
- **AIB, Inc.**: Commentary documents active plan — team aware
- **EverCommerce Solutions Inc.**: Open Gainsight CTA covers this play: "Activity Timeline - Zuora + EverCommerce: Billing Scenarios — EverCommerce Solutions Inc."
- **Gfi USA LLC**: Commentary documents active plan — team aware
- **Secureframe**: Commentary documents active plan — team aware
- **The Columbian Publishing Company**: Commentary documents active plan — team aware
- **Urban Science Applications Inc.**: Customer mid-migration/RFP — paused vendor activity per commentary
- **BTS USA, Inc. fka Rapid Learning Institute**: Commentary documents active plan — team aware
- **GoCanvas**: Commentary documents active plan — team aware
- **Dealers United LLC**: Commentary documents active plan — team aware
- **Quotit Corporation**: Commentary documents active plan — team aware
- **Bamboo Rose**: Commentary documents active plan — team aware
- **DaySmart Software**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Zengine Ltd fka IntelliCentrics Inc.**: Commentary documents active plan — team aware
- **Weiss Ratings, LLC**: Commentary documents active plan — team aware
- **Malwarebytes, Inc. (restart)**: Commentary documents active plan — team aware
- **Tobii Dynavox**: Commentary documents active plan — team aware
- **Teladoc Health, Inc.**: Commentary documents active plan — team aware
- **iRobot Corporation**: Commentary documents active plan — team aware
- **TitanFile Inc.**: Commentary documents active plan — team aware
- **View The Space, Inc.**: Commentary documents active plan — team aware
- **TrainingPeaks**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Asset Panda**: Open Gainsight CTA covers this play: "Activity Timeline - Asset Panda/Zuora Meeting  — Asset Panda"
- **Hitachi Vantara**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Aviat U.S., Inc.**: Commentary documents active plan — team aware
- **Home Care Pulse**: Commentary documents active plan — team aware
- **Otter.ai, Inc**: Commentary documents active plan — team aware
- **Data Doctors Quality Care, LLC**: Commentary documents active plan — team aware
- **Simpleview Inc (DTN BU uses Zuora)**: Commentary documents active plan — team aware
- **Donnelley Financial LLC**: Commentary documents active plan — team aware
- **A10 Networks, Inc.**: Commentary documents active plan — team aware
- **Crunchbase, Inc.**: Commentary documents active plan — team aware
- **Telestream**: Commentary documents active plan — team aware
- **WABTEC: WESTINGHOUSE AIR BRAKE TECHNOLOGIES CORPORATION**: Commentary documents active plan — team aware
- **MarginEdge**: Customer mid-migration/RFP — paused vendor activity per commentary
- **OfferUp**: Commentary documents active plan — team aware
- **Xactware Solutions, Inc.**: Commentary documents active plan — team aware
- **UL Verification Services Inc**: Commentary documents active plan — team aware
- **Aabaco Small Business, LLC**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Wondrium (Teaching Company)**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Flowcode**: Commentary documents active plan — team aware
- **Open Education LLC.**: Customer mid-migration/RFP — paused vendor activity per commentary
- **Teledyne FLIR, LLC**: Commentary documents active plan — team aware
- **Dark Matter Technologies, Inc.**: Commentary documents active plan — team aware
- **Pipedrive, Inc.**: Customer mid-migration/RFP — paused vendor activity per commentary
