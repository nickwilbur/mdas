...
import { getWoWChangeEvents } from '@/lib/read-model';
...
export default async function WoWPage() {
  const { events, prevId, currId } = await getWoWChangeEvents();
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-gray-500">
        Diff between {prevId ? prevId.slice(0, 8) : '—'} and {currId ? currId.slice(0, 8) : '—'} ({events.length} events)
      </p>
