...
import { parseCloseGapActionSteps } from './forecast-close-gap-plan-core';
...
function buildPrompt(
  ...
  return [
    ...
    ``,
    `OUTPUT FORMAT — strict JSON array, no markdown, no preamble. 2–4 steps, ordered by leverage (highest-impact action first). Each step:`,
    `  {"owner": "${primaryOwner}", "action": "<concrete next move, ≤180 chars, ideally with a timeframe>"}`,
    ``,
